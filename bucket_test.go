...
func TestBucketPut(t *testing.T) {
	...
	for _, p := range testCases {
		ok, err := b.Put(p, nil)
		if err != nil {
			t.Fatalf("An error occurs when putting a pair to the bucket: %s (pair: %#v)", err, p)
		}
		if !ok {
			t.Fatalf("Couldn't put pair to the bucket! (pair: %#v)", p)
		}
		...
		ok, err = b.Put(p, nil)
		if err != nil {
			t.Fatalf("An error occurs when putting a repeated pair to the bucket: %s (pair: %#v)", err, p)
		}
		if ok {
			t.Fatalf("Couldn't put repeated pair to the bucket! (pair: %#v)", p)
		}
		count++
		if b.Size() != count {
...
func TestBucketClear(t *testing.T) {
	...
	b.Clear(nil)
	current := b.Get(testCases[0].Key())
	if current != nil {
		t.Fatalf("Couldn't clear bucket, pair:%#v", current)
	}
	if b.Size() != 0 {
		t.Fatalf("Inconsistent size: expected: %d, actual: %d",
			0, b.Size())
